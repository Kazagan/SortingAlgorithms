...
int str_equality(const void * expected,const void * actual);
...
int main() {
    ...
    /* str_equality(a, b); */
    assert_array(sizeof(char), ordered_str, 10, test_str, 10, str_equality);
    return 0;
}
...
int str_equality(const void * expected,const void * actual) {
    int i = 0;
    char *x = *(char**)expected;
    char *y = *(char**)actual;
    while(x[i] != '\0') {
        if(x[i] != y[i]) {
            fprintf(stderr, "%s differs from %s at '%c' not equal to '%c'\n", x, y, x[i], y[i]);
            return 0;
        }
        i++;
    }
